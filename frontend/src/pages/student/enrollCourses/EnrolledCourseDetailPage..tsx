...
import { getSpecificCourse, markChapterAsCompleted, checkChapterCompletedOrNot } from "../../../api/action/StudentAction";
...
const EnrolledCourseDetailPage = () => {
    ...
    const fetchCourseDetails = async () => {
      try {
        ...
        setCompletedChapterIds(
          response.enrollment.completedChapters.map((c: { chapterId: string }) => c.chapterId)
        );
      } catch (err) {
  ...
  const calculateProgress = () => {
    ...
    return Math.round((completedChapterIds.length / enrollment.courseId.chapters.length) * 100);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <nav className="bg-white shadow-sm px-6 py-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => navigate('/')}>...</div>
      ...
      <div className="flex h-[calc(100vh-80px)]">
        <aside className={`${sidebarCollapsed ? 'w-16' : 'w-80'} bg-white shadow-lg flex flex-col transition-all duration-300 ease-in-out`}>
          <div className="p-4 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className={`font-bold text-gray-800 ${sidebarCollapsed ? 'hidden' : 'text-lg'}`}>
                {course.courseName}
              </h2>
              <button
                      ...
                      <div className="p-3 flex items-center space-x-3">
                        ...
                        {!sidebarCollapsed && (
                          <div className="flex-1 min-w-0">
                            <p className={`font-medium truncate ${isCurrent ? 'text-white' : 'text-gray-800'}`}>
        ...
        <main className="flex-1 flex flex-col bg-white">
          <div className="p-6 border-b border-gray-200 bg-white">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600 mt-1">Chapter {currentChapterIndex + 1} of {course.chapters.length}</p>
              </div>
              <div className="flex items-center space-x-3">
                {completedChapterIds.includes(currentChapter._id) ? (
                  <span className="px-4 py-2 rounded-full text-sm font-medium bg-green-100 text-green-800">Completed</span>
                ) : (
                  <button
                    onClick={() => handleMarkCompleted(currentChapter._id)}
          ...
          <div className="p-6 border-t border-gray-200 bg-white flex justify-between items-center">
            ...
            <div className="text-sm text-gray-600">
              Chapter {currentChapterIndex + 1} of {course.chapters.length} — {progress}% Complete
            </div>
