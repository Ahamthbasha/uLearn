// // DataTable.tsx - Updated with built-in pagination
...
//                         {pagination ? (pagination.currentPage - 1) * data.length + index + 1 : index + 1}
//                       </td>
//                       {columns.map((column) => (
...
export interface DataTableProps<T = any> {
  ...
  onSearchChange?: (value: string) => void;
  searchPlaceholder?: string;
  leftSideHeaderContent?: React.ReactNode; // NEW PROP
}
...
const DataTable = <T extends Record<string, any>>({
  ...
  onSearchChange,
  searchPlaceholder = "Search...",
  leftSideHeaderContent, // NEW PROP
}: DataTableProps<T>) => {
  const resolveValue = <K,>(value: K | ((record: T) => K), record: T): K => {
    return typeof value === 'function' ? (value as (record: T) => K)(record) : value;
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 pt-12 lg:pt-0">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            {/* Right Side: Search and Optional Button */}
